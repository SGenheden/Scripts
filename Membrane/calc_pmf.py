...
if __name__ == '__main__' :
  ...
  if not args.zconst :
    ...
    print "Minimum overlap is %.2f between %s and %s"%(overlap.min(),args.files[np.argmin(overlap)],args.files[np.argmin(overlap)+1])
    ...
    for i,sim in enumerate(simulations) : 
      ...
      overlap = sim.pairwise_overlap()
      print "Minimum block overlap is %.2f between %s and %s"%(overlap.min(),args.files[np.argmin(overlap)],args.files[np.argmin(overlap)+1])
